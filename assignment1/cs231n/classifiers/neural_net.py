...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        # shape (N, C).                                                             #
        #############################################################################
        temp_scores = X.dot(W1) + b1
        
        relu = np.maximum(0, temp_scores)
        ...
        # calculate loss
        loss = np.sum(-np.log(up / down)) / num_train + reg * np.sum(W1 ** 2) + reg * np.sum(W2 ** 2)
        pass
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################
        ...
        dz2 = temp  #db2 (in graph)
        dz2 = dz2/z2_length #softmax
        dW2 = relu.T.dot(dz2) 
        dW2 += 2*reg*W2
        #Keep cont to Relu
        dh1 = dz2.dot(W2.T)
        ...
        dW1 = X.T.dot(dz1)
#         dW1 /= N?
        dW1 += 2*reg*W1 #from the first derivation
    ...
    def train(self, X, y, X_val, y_val,
        ...
        for it in range(num_iters):
            ...
            learning_rete = (1/(1+learning_rate_decay*it))*learning_rate
            pass
            #########################################################################
            #                             END OF YOUR CODE                          #
            #########################################################################
    ...
    def predict(self, X):
        ...
        ###########################################################################
        scores = self.loss(X, y = None, reg = 0)
